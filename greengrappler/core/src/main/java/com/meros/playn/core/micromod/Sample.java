...
public class Sample {
	...
	private static final int TABLE_INTERP_SHIFT = FP_SHIFT
			- LOG2_TABLE_ACCURACY;
	...
	public void setSampleData(short[] sampleData, int loopStart,
			int loopLength, boolean pingPong) {
		...
		loopStart += DELAY;
		// Allocate new sample.
		int newSampleLength = DELAY + sampleLength
				+ (pingPong ? loopLength : 0) + FILTER_TAPS;
	...
	public void resampleSinc(int sampleIdx, int sampleFrac, int step,
		...
		while (outIdx < outEnd) {
			...
			a2 >>= FP_SHIFT;
			int y = a1
					+ ((a2 - a1) * (sampleFrac & TABLE_INTERP_MASK) >> TABLE_INTERP_SHIFT);
			mixBuffer[outIdx++] += y * leftGain >> FP_SHIFT;
			mixBuffer[outIdx++] += y * rightGain >> FP_SHIFT;
			sampleFrac += step;
			sampleIdx += sampleFrac >> FP_SHIFT;
