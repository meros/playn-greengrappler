...
public class Boss extends Entity {
	...
	private int myHealth = UtilMethods.lerp(MAX_HEALTH, MIN_HEALTH,
			PlayerSkill.get());

	enum State {
		INIT, SLEEPING, INITIAL_BLOW, AWEKENING, MOVE_UPWARDS, FLOAT, STOP, ATTACK, VULNERABLE, DEAD
	}
	...
	private Animation myReactor = Resource.getAnimation(
			"data/images/reactor_shell.bmp", 4);
	...
	public void draw(Surface buffer, int offsetX, int offsetY, int layer) {
		...
		if (myState == State.SLEEPING)
			myReactor.drawFrame(buffer, DAMAGE_MAX / FRAME_PER_DAMAGE
					- (myInitalHealth + 3) / FRAME_PER_DAMAGE, x, y);
	...
	public void update() {
		...
		if (myState != State.MOVE_UPWARDS && myState != State.SLEEPING
				&& myState != State.INITIAL_BLOW && myState != State.DEAD
		...
		if (myState == State.MOVE_UPWARDS) {
			...
			if (myOriginalPos.getY() - getPosition().getY() > 130) {
				myState = State.FLOAT;
				myRoom.broadcastBoosWallActivate();
				myFrameCounter = 0;
			}
		}
			...
			if (myFrameCounter < 40) {
				if (myFrameCounter % 2 == 0)
					setPosition(getPosition()
							.add(new ImmutableFloatPair(-2, 0)));
				else
				...
				if (myOriginalPos.getY() < getPosition().getY()) {
					myRoom.broadcastBoosWallDectivate();
					setPosition(new ImmutableFloatPair(getPosition().getX(),
							myOriginalPos.getY()));
					myState = State.VULNERABLE;
					myRoom.getCamera().addShake(5.0f, 80);

					int numPs = random.nextInt() % 10 + 5;
					ParticleSystem ps = new ParticleSystem(
							Resource.getAnimation("data/images/debris.bmp", 4),
							10, 40, 10, 1, 50, numPs, new ImmutableFloatPair(
									0.0f, -20.0f), 2.0f);
					ps.setPosition(getPosition(), 10.0f, false);
					myRoom.addEntity(ps);
					myRoom.broadcastBoosFloorActivate();
					Sound.playSample("data/sounds/reactor_explosion");
					Coin.SpawnDeathCoins(random.nextInt() % 5 + 3,
							getPosition(), 60 * 6, myRoom);
				}
			}
	...
	public void onDamage() {
		if (myState == State.SLEEPING) {
			...
			if (myInitalHealth == 0) {
				...
				myRoom.getCamera().addShake(4.0f, INITIAL_BLOW_TIME);
			} else {
				Sound.playSample("data/sounds/damage");
