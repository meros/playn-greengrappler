...
public class ParticleSystem extends Entity {
	...
	public ParticleSystem(Animation aAnimation, int aAnimationSpeed,
			...
			int aNumParticles, float2 aInitialVel, float aGravity) {
	...
	public void draw(Canvas aBuffer, int aOffsetX, int aOffsetY, int aLayer) {
		...
		for (int i = 0; i < myParticlesPos.size(); i++) {
			myAnimation.drawFrame(aBuffer, myFrameNum / myAnimationSpeed
					+ random.nextInt(), (int) (myParticlesPos.get(i).x
					+ aOffsetX - myAnimation.getFrameWidth() / 2),
					(int) (myParticlesPos.get(i).y + aOffsetY - myAnimation
							.getFrameHeight() / 2));
		}
	}
	...
	public void setPosition(float2 position, float randomAmount,
			boolean randomVel) {
		super.setPosition(position);

		for (int i = 0; i < myNumParticles; i++) {
			float2 pos = getPosition();
			pos = pos.add(new float2(
			...
			float2 velocity = randomVel ? new float2(1, 1).multiply(r0710)
