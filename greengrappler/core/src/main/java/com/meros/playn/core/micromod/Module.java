...
public class Module {
	...
	private void loadS3M(byte[] moduleData) {
		...
		for (int instIdx = 1; instIdx <= numInstruments; instIdx++) {
			...
			if (moduleData[instOffset] != 1)
				continue;
			if (ushortle(moduleData, instOffset + 76) != 0x4353)
				continue;
			...
			if (sixteenBit) {
				if (signedSamples) {
					...
					for (int idx = 0, end = sampleData.length; idx < end; idx++) {
						int sam = (moduleData[sampleOffset] & 0xFF)
	...
	private void loadXM(byte[] moduleData) {
		...
		boolean deltaEnv = isoLatin1(moduleData, 38, 20).startsWith(
				"DigiBooster Pro");
		...
		for (int patIdx = 0; patIdx < numPatterns; patIdx++) {
			...
			Pattern pattern = patterns[patIdx] = new Pattern(numChannels,
					numRows);
			...
			if (patternDataLength > 0) {
				...
				for (int note = 0; note < numNotes; note++) {
					...
					if ((flags & 0x80) == 0)
						...
						dataOffset++;
					byte key = (flags & 0x01) > 0 ? moduleData[dataOffset++]
							: 0;
					pattern.data[patternDataOffset++] = key;
					byte ins = (flags & 0x02) > 0 ? moduleData[dataOffset++]
							: 0;
					pattern.data[patternDataOffset++] = ins;
					byte vol = (flags & 0x04) > 0 ? moduleData[dataOffset++]
							: 0;
		...
		for (int insIdx = 1; insIdx <= numInstruments; insIdx++) {
			...
			int numSamples = instrument.numSamples = ushortle(moduleData,
					dataOffset + 27);
			if (numSamples > 0) {
				...
				for (int point = 0; point < 12; point++) {
					...
					pointTick = (deltaEnv ? pointTick : 0)
							+ ushortle(moduleData, pointOffset);
					volEnv.pointsTick[point] = pointTick;
					volEnv.pointsAmpl[point] = ushortle(moduleData,
							pointOffset + 2);
				}
				...
				for (int point = 0; point < 12; point++) {
					...
					panEnv.pointsAmpl[point] = ushortle(moduleData,
							pointOffset + 2);
				}
				...
				instrument.volumeFadeOut = ushortle(moduleData,
						dataOffset + 239);
			}
			dataOffset += intle(moduleData, dataOffset);
			...
			for (int samIdx = 0; samIdx < numSamples; samIdx++) {
				...
				sample.name = codePage850(moduleData, sampleHeaderOffset + 18,
						22);
