	...
	void privInit(Image aGlyphImage, String aGlyphs) {
		...
		while(scanLine < glyphImage.height())
		{
			...
			while(x < glyphImage.width())
			{
				...
				{
					int y1 = scanLine;
					while((y1 < glyphImage.height()) && getpixel(glyphImage, x, y1) != separatingColor) //find bottom of glyph
					{
						y1++;
					}
					int x1 = x;
					while((x1 < glyphImage.width()) && getpixel(glyphImage, x1, scanLine) != separatingColor) //find right edge of glyph
					{
						x1++;
					}
					...
					char currGlyph = aGlyphs.length() <= currGlyphIndex ? specialGlyph++ : aGlyphs.charAt(currGlyphIndex++);
					Image bitmap = glyphImage.subImage(x, scanLine, width, height);
		...
		if(!myGlyphToBitmap.containsKey(""+'\n') && myGlyphToBitmap.containsKey(""+' ')) 
			myGlyphToBitmap.put(""+'\n', myGlyphToBitmap.get(""+' ')); //have something to draw when newline
