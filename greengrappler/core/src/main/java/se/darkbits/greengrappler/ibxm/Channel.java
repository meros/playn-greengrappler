...
public class Channel {
	...
	public void resample(int[] outBuf, int offset, int length, int interpolation) {
		...
		case NEAREST:
			sample.resampleNearest(sampleIdx, sampleFra, step, lAmpl, rAmpl,
					outBuf, offset, length);
			break;
		case LINEAR:
		default:
			sample.resampleLinear(sampleIdx, sampleFra, step, lAmpl, rAmpl,
					outBuf, offset, length);
			break;
		case SINC:
			sample.resampleSinc(sampleIdx, sampleFra, step, lAmpl, rAmpl,
					outBuf, offset, length);
			break;
		}
	}
	...
	public void updateSampleIdx(int length) {
		...
		sampleIdx = sample.normaliseSampleIdx(sampleIdx
				+ (sampleFra >> Sample.FP_SHIFT));
	...
	public void row(Note note) {
		...
		if (noteEffect != 0x7D && noteEffect != 0xFD)
			trigger();
		switch (noteEffect) {
		case 0x01:
		case 0x86: /* Porta Up. */
	...
	public void tick() {
		...
		case 0x84: /* Vol Slide. */
			...
			break;
		case 0x11: /* Global Volume Slide. */
			globalVol.volume += (globalVslideParam >> 4)
					- (globalVslideParam & 0xF);
	...
	private void autoVibrato() {
		...
		if (depth > 0) {
			...
			vibratoAdd += waveform(autoVibratoCount * rate >> 2, type + 4)
					* depth >> 8;
			autoVibratoCount++;
		}
	}
	...
	private void vibrato(boolean fine) {
		vibratoAdd = waveform(vibratoPhase, vibratoType & 0x3) * vibratoDepth >> (fine ? 7
				: 5);
	}
	...
	private void calculateFrequency() {
		if (module.linearPeriods) {
			...
			if (freq < 65536)
				step = (freq << Sample.FP_SHIFT) / sampleRate;
			else
				step = (freq << (Sample.FP_SHIFT - 3)) / (sampleRate >> 3);
		} else {
	...
	private void trigger() {
		if (noteIns > 0 && noteIns <= module.numInstruments) {
			...
			Sample sam = instrument.samples[instrument.keyToSample[noteKey < 97 ? noteKey
					: 0]];
